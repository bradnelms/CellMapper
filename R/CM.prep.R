...
function(Data, DataSource = '', GeneIDType = '', verbose = TRUE) {
	...
	if (!(is.character(DataSource) & is.character(GeneIDType) & (length(DataSource) == 1) & (length(GeneIDType) == 1))) {
		stop("'DataSource' and 'GeneIDType' must both be character vectors of length = 1.")
	}
	...
	out <- CellMapperList(B = t(sv$u), d = sv$d, meta = list(nrow = dim(dataMat)[1], ncol = dim(dataMat)[2], GeneIDType = GeneIDType, DataSource = DataSource))
